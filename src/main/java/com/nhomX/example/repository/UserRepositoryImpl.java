...
public class UserRepositoryImpl implements UserRepository {
  ...
  public boolean register(User user) {
    /*
     * Cột username được khai báo UNIQUE chính là chìa khóa. Nghĩa là SQLite:
     * "Tuyệt đối không bao giờ được cho phép 2 dòng có cùng giá trị ở cột này". Khi code Java cố
     * tình chạy lệnh INSERT với một email đã tồn tại, Database sẽ lập tức từ chối bản ghi đó và ném
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
      try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
        ...
        if (rowsAffected > 0) {
          logger.info("Đã tạo tài khoản thành công cho: {}", user.getUserName());
          return true;
        }
      }
      ...
      if (e.getMessage() != null && e.getMessage().contains("UNIQUE constraint failed")) {
        logger.warn("Email đã tồn tại: {}", user.getUserName());
      } else {
        logger.error("Lỗi đăng ký user", e);
      }
    }
    return false;
  }
  ...
  public User findById(String id) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      logger.error("Lỗi tìm kiếm user", e);
    }
    return null;
  }
  ...
  public void updateBalance(String userId, long deltaAmount) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      if (rows == 0) {
        logger.warn("Cập nhật số dư thất bại: số dư không đủ hoặc user không tồn tại");
      }
    } catch (SQLException e) {
  ...
  public User findByUsername(String username) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      logger.error("Lỗi tìm user theo username", e);
    }
    return null;
  }
  ...
  public List<User> findAll() {
    ...
    try(Connection conn = DatabaseConnection.getInstance().getConnection()){
      ...
      try (PreparedStatement pstmt = conn.prepareStatement(sql);
        ...
        logger.error("Lỗi lấy danh sách user", e);
      }

    } catch (SQLException e) {
      // Bắt lỗi tổng (ví dụ lỗi không mở được file database)
      logger.error("Lỗi khởi tạo kết nối DB", e);
    }
  ...
  public boolean updatePassword(String username, String newPasswordHash) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      logger.error("Lỗi khi cập nhật mật khẩu", e);
    }
    return false;
  }
  ...
  public int getTotalUserCount() {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection();
      ...
      logger.error("Lỗi getTotalUserCount", e);
    }
    return count;
  }
  ...
  public boolean setUserActiveStatus(String userId, boolean isActive) {
    ...
    try(Connection conn = DatabaseConnection.getInstance().getConnection()) {
      try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
        ...
        int rows = pstmt.executeUpdate();
        if (rows > 0) {
          logger.info("Đã {} tài khoản: {}", isActive ? "mở khóa" : "khóa", userId);
          return true;
        }
        logger.warn("Không tìm thấy user để cập nhật trạng thái: {}", userId);
      }
    } catch (SQLException e) {
      logger.error("Lỗi setUserActiveStatus", e);
    }
    return false;
  }
  ...
  public boolean deleteUser(String userId) {
    ...
    try (Connection conn = DatabaseConnection.getInstance().getConnection()) {
      ...
      try {
        ...
        executeUpdate(conn, "DELETE FROM item_images WHERE item_id IN "
            + "(SELECT id FROM items WHERE seller_id = ?)", userId);

        // Bước 4: Xóa auctions của các items thuộc user
        // (cần trước khi xóa items)
        executeUpdate(conn,
            "DELETE FROM auctions WHERE item_id IN " + "(SELECT id FROM items WHERE seller_id = ?)",
            userId);
        ...
        conn.commit();
        if (rows > 0) {
          logger.info("Đã xóa toàn bộ dữ liệu của user: {}", userId);
          return true;
        }
        logger.warn("Không tìm thấy user để xóa: {}", userId);
        return false;

      } catch (SQLException e) {
        logger.error("Lỗi deleteUser - đang rollback", e);
      ...
      logger.error("Lỗi mở kết nối DB khi xóa User", e);
      return false;
    }
  }
  ...
  public boolean isUserActive(String userId) {
    ...
    try(Connection conn = DatabaseConnection.getInstance().getConnection()) {
      ...
      logger.error("Lỗi isUserActive", e);
    }
